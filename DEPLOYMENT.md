...
source. So we run a **dedicated go2rtc** (`kustomize/go2rtc/`) using go2rtc's **native
...
1. **Fill `kustomize/secrets/go2rtc.env`** (copy from `go2rtc.env.example`):
   - `RING_REFRESH_TOKEN` — generate from a **separate** Ring login than ring-mqtt's
     (two clients sharing one token rotate each other out). Easiest: port-forward go2rtc
     once it's up (`kubectl port-forward deploy/go2rtc 1984 -n home-automation`), open
...
2. **Edit `kustomize/go2rtc/configmap.yaml`** so each `streams:` entry is named **exactly
   the HA camera base** (`camera.<base>` → `<base>`); the app derives the go2rtc `src` from
   it. Add one line per camera.
3. `kubectl apply -k kustomize/` (then `kubectl rollout restart deploy/go2rtc` after any
   secret edit — stable secret names don't auto-roll).
